...
salbutamol_inhaler_data_table <- salbutamol_inhaler_data %>%
  ...
  fmt_number(columns = quantity_per_1k,
    decimals = 0) %>% 
  opt_stylize()
...
line_graph <- ggplot(salbutamol_prescriptions_2024, aes(x = season, y = total_quantity_per_1k, group = 1, text = paste(
  "Season: ", season, "<br>",
  "Total Prescriptions per 1,000: ", scales::comma(total_quantity_per_1k, accuracy = 1)
))) +
  geom_line(color = "darkgreen", linewidth = 1) +
  geom_point() +
  labs(x = "Season", y = "Total Salbutamol Inhaler Prescriptions per 1,000 people", title = "Seasonal Salbutamol Prescriptions per 1,000 people in 2024") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10))

plotly_graph <- ggplotly(line_graph, tooltip = "text") %>% 
...
seasonal_salbutamol_prescriptions <- salbutamol_inhaler_data %>% 
  ...
  summarise(avg_paid_quantity = sum(paid_quantity, na.rm = TRUE)) %>% 
  summarise(total_seasonal_quantity_per_1k = (avg_paid_quantity/hb_population)*1000)

line_chart <- ggplot(seasonal_hb_totals, aes(x = season, y = total_quantity_per_1k, group = hb_name
  )) +
  geom_line(color = "darkgreen", linewidth = 1) +
  geom_point() +
  facet_wrap(~ hb_name, ncol = 3, scales = "free") +
  labs(x = "Season", y = "Total Salbutamol Inhaler Prescriptions per 1,000 people", title = "Seasonal Salbutamol Prescriptions by Health Board", subtitle = "Prescriptions per 1,000 people") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10))
...
plotly_map <- ggplotly(map_salbutamol_inhaler_per_1k, tooltip = "text") %>% 
  config(displayModeBar = FALSE)
plotly_map
