...
This report aims to investigate the trends regarding prescribing of salbutamol inhalers in Scotland in the year 2024. With a focus on variations across the different NHS Health Boards, and across different seasons throughout the year.
...
seasonal_salbutamol_prescriptions <- salbutamol_inhaler_data %>% 
  ...
  summarise(avg_quantity_per_100k = (avg_paid_quantity/hb_population)*100000)


line_graph <- ggplot(seasonal_salbutamol_prescriptions, aes(x = season, y = avg_quantity_per_100k, group = hb_name)) +
  geom_line(color = "darkgreen", linewidth = 1) +
  geom_point() +
  facet_wrap(~ hb_name, ncol = 3, scales = "free") +
  labs(x = "Season", y = "Average Quantity per 100,000", title = "Seasonal Salbutamol Prescribing by Health Board") +
  theme_bw() +
...
salbutamol_inhaler_data_hb <- NHS_healthboards %>%
...
map_salbutamol_inhaler_data_hb <- salbutamol_inhaler_data_hb %>%
...
map_salbutamol_inhaler_data_hb
